...
func (c *clientListener) NewBlockAvailable(kapi *AstrixApi) {
	c.clientLock.Lock()
		...
		go func(client *gostratum.StratumContext) {
			...
			if err != nil {
				if strings.Contains(err.Error(), "Could not decode address") {
					...
					client.Logger.Error(fmt.Sprintf("failed fetching new block template from astrix, malformed address: %s", err))
	...
	if time.Since(c.lastBalanceCheck) > balanceDelay {
		...
		if len(addresses) > 0 {
			go func() {
				balances, err := kapi.astrixd.GetBalancesByAddresses(addresses)
				if err != nil {
					c.logger.Warn("failed to get balances from astrix, prom stats will be out of date", zap.Error(err))
					return
				}
